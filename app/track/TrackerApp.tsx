...
type Section = "overview" | "inventory" | "transactions" | "expenses" | "analytics" | "calculator";
...
function Sidebar({ section, onNavigate }: { section: Section; onNavigate: (section: Section) => void }) {
  return <aside className="track-sidebar">
    ...
    <button className={section === "calculator" ? "track-calculator-link active" : "track-calculator-link"} type="button" onClick={() => onNavigate("calculator")}><span>06</span>Calculator</button>
...
function Overview({ data, onNavigate, onSale }: { data: OverviewData; onNavigate: (section: Section) => void; onSale: () => void }) {
  ...
  return <><SectionHeader kicker="Command centre" title="Overview" detail="Trading performance, actual operating spend and capital still in stock." actions={<button className="track-button-primary" type="button" onClick={onSale}>＋ Record sale</button>} />
    <div className="track-metric-grid"><Metric index="01" label="Net profit" value={formatCompactMoney(data.metrics.netProfitOre)} detail="Trading profit − operating expenses" tone={data.metrics.netProfitOre < 0 ? "negative" : "positive"} /><Metric index="02" label="Trading profit" value={formatCompactMoney(data.metrics.tradingProfitOre)} detail="Sales after trading costs" tone={data.metrics.tradingProfitOre < 0 ? "negative" : "positive"} /><Metric index="03" label="Operating expenses" value={formatCompactMoney(data.metrics.operatingExpensesOre)} detail="Expenses + recorded subscriptions" /><Metric index="04" label="Revenue" value={formatCompactMoney(data.metrics.revenueOre)} detail="Gross product sales" /></div>
    <div className="track-capital-strip"><div><span>Inventory value</span><strong>{formatMoney(data.metrics.inventoryValueOre)}</strong><small>Remaining landed cost</small></div><div><span>Cash invested</span><strong>{formatMoney(data.metrics.cashInvestedOre)}</strong><small>Lifetime purchase outlay</small></div><button type="button" onClick={() => onNavigate("expenses")}><span>Operating ledger</span><strong>Open expenses →</strong><small>Review actual business spend</small></button></div>
    <div className="track-overview-grid"><section className="track-panel track-chart-panel"><div className="track-panel-heading"><div><p className="track-kicker">Bottom line</p><h2>Net profit over time</h2></div><span>ALL</span></div>{series.length ? <><ProfitChart points={series} label="Cumulative net profit after operating expenses" /><div className="track-chart-axis"><span>{formatDate(series[0].date)}</span><strong>{formatMoney(series.at(-1)?.value ?? 0)}</strong><span>{formatDate(series.at(-1)?.date ?? series[0].date)}</span></div></> : <EmptyState code="00" title="No realised result yet" detail="Sales and actual operating expenses build the chart." action={<button type="button" className="track-text-button" onClick={onSale}>Record a sale →</button>} />}</section>
      <section className="track-panel"><div className="track-panel-heading"><div><p className="track-kicker">Ledger</p><h2>Recent activity</h2></div><button type="button" className="track-text-button" onClick={() => onNavigate("transactions")}>View trading →</button></div>{data.recentActivity.length ? <div className="track-activity-list">{data.recentActivity.map((entry) => { const outgoing = entry.kind !== "SALE"; return <article key={entry.id}><span className={`track-activity-mark ${entry.kind.toLowerCase().replace("_", "-")}`}>{entry.kind === "SALE" ? "↗" : entry.kind === "PURCHASE" ? "↓" : "−"}</span><div><strong>{entry.title}</strong><small>{entry.detail}</small></div><div><strong className={outgoing ? "negative" : ""}>{outgoing ? "−" : ""}{formatMoney(entry.amountOre)}</strong><small>{formatDate(entry.occurredAt)}</small></div></article>; })}</div> : <EmptyState code="00" title="No activity yet" detail="Trading and operating entries will form your private ledger." />}</section></div>
...
function Analytics({ data, period, onPeriod }: { data: AnalyticsData; period: AnalyticsPeriod; onPeriod: (period: AnalyticsPeriod) => void }) {
  ...
  return <><SectionHeader kicker="Performance layer" title="Analytics" detail="Trading performance and real operating spend, without forecasted subscription costs." actions={<div className="track-periods">{(["30D", "90D", "YTD", "ALL"] as const).map((item) => <button className={period === item ? "active" : ""} type="button" key={item} onClick={() => onPeriod(item)}>{item}</button>)}</div>} />
    <div className="track-analytics-metrics"><Metric index="01" label="Revenue" value={formatCompactMoney(data.totals.revenueOre)} detail={`${data.totals.unitsSold} units sold · ${period}`} /><Metric index="02" label="Trading profit" value={formatCompactMoney(data.totals.tradingProfitOre)} detail={`${formatPercent(data.totals.tradingProfitOre, data.totals.revenueOre)} trading margin`} tone={data.totals.tradingProfitOre < 0 ? "negative" : "positive"} /><Metric index="03" label="Operating expenses" value={formatCompactMoney(data.totals.operatingExpensesOre)} detail="Actual recorded spend" /><Metric index="04" label="Net profit" value={formatCompactMoney(data.totals.netProfitOre)} detail="Trading profit − operating expenses" tone={data.totals.netProfitOre < 0 ? "negative" : "positive"} /></div>
    <div className="track-analytics-grid"><section className="track-panel track-chart-panel"><div className="track-panel-heading"><div><p className="track-kicker">Bottom line</p><h2>Net profit over time</h2></div><span>{period}</span></div>{cumulative.length ? <><ProfitChart points={cumulative} label={`Cumulative net profit for ${period}`} /><div className="track-chart-axis"><span>{formatDate(cumulative[0].date)}</span><strong>{formatMoney(cumulative.at(-1)?.value ?? 0)}</strong><span>{formatDate(cumulative.at(-1)?.date ?? cumulative[0].date)}</span></div></> : <EmptyState code="00" title="No financial activity in this period" detail="Choose another period or record a sale or expense." />}</section><section className="track-panel track-comparison"><div className="track-panel-heading"><div><p className="track-kicker">Economic bridge</p><h2>Revenue and costs</h2></div></div><div className="track-comparison-bars"><article><div><span>Revenue</span><strong>{formatMoney(data.totals.revenueOre)}</strong></div><i><b style={{ width: `${(data.totals.revenueOre / maxComparison) * 100}%` }} /></i></article><article className="cost"><div><span>Trading costs</span><strong>{formatMoney(data.totals.tradingCostsOre)}</strong></div><i><b style={{ width: `${(data.totals.tradingCostsOre / maxComparison) * 100}%` }} /></i></article><article className="operating"><div><span>Operating expenses</span><strong>{formatMoney(data.totals.operatingExpensesOre)}</strong></div><i><b style={{ width: `${(data.totals.operatingExpensesOre / maxComparison) * 100}%` }} /></i></article></div><dl><div><dt>Trading profit</dt><dd>{formatMoney(data.totals.tradingProfitOre)}</dd></div><div><dt>Operating expenses</dt><dd>−{formatMoney(data.totals.operatingExpensesOre)}</dd></div><div><dt>Net profit</dt><dd className={data.totals.netProfitOre >= 0 ? "positive" : "negative"}>{formatMoney(data.totals.netProfitOre)}</dd></div></dl></section></div>
...
export default function TrackerApp() {
  ...
  const loadCore = useCallback(async (initial = false) => {
    ...
    try { const [overviewResult, inventoryResult, transactionResult, expenseResult] = await Promise.all([
    ...
    ]); setOverview(overviewResult); setInventory(inventoryResult.products); setTransactions(transactionResult.transactions); setExpenses(expenseResult); }
