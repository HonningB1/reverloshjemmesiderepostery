...
import { TrackerI18nProvider, TrackerLanguageSelector, translate, useTrackerI18n } from "./i18n";
...
type Section = "overview" | "inventory" | "transactions" | "expenses" | "vat" | "analytics" | "calculator";
...
const navigation: Array<{ id: Section; label: string }> = [
  ...
  { id: "transactions", label: "Transactions" }, { id: "expenses", label: "Expenses" },
...
const emptyAnalytics: AnalyticsData = {
  period: "30D", totals: { unitsSold: 0, revenueOre: 0, costBasisOre: 0, tradingCostsOre: 0, tradingProfitOre: 0, operatingExpensesOre: 0, netProfitOre: 0 },
  series: [], products: [],
};
const emptyExpenses: ExpensesData = {
  expenses: [], subscriptions: [], payments: [],
  totals: { ordinaryExpensesOre: 0, subscriptionExpensesOre: 0, operatingExpensesOre: 0, activeSubscriptions: 0 },
};
const emptyVat: VatData = {
  totals: { inputVatOre: 0, deductibleInputVatOre: 0, outputVatOre: 0, paidSettlementsOre: 0, receivedSettlementsOre: 0, openPositionOre: 0, receivableOre: 0, payableOre: 0 },
  settlements: [],
};

function currentNumberLocale() { return typeof window !== "undefined" && window.localStorage.getItem("reverlo-tracker-locale") === "da" ? "da-DK" : "en-DK"; }
function formatMoney(ore: number) { return new Intl.NumberFormat(currentNumberLocale(), { style: "currency", currency: "DKK", minimumFractionDigits: 2 }).format(ore / 100); }
function formatCompactMoney(ore: number) { return Math.abs(ore) >= 100_000_00 ? new Intl.NumberFormat(currentNumberLocale(), { style: "currency", currency: "DKK", notation: "compact", maximumFractionDigits: 1 }).format(ore / 100) : formatMoney(ore); }
function formatDate(value: string) { const date = new Date(`${value}T00:00:00Z`); return Number.isNaN(date.getTime()) ? value : new Intl.DateTimeFormat(currentNumberLocale() === "da-DK" ? "da-DK" : "en-GB", { day: "2-digit", month: "short", year: "numeric" }).format(date); }
...
function statusLabel(status: TrackerStatus) {
  const english = status === "IN_STOCK" ? "In stock" : status === "LISTED" ? "Listed" : status === "RESERVED" ? "Reserved" : "Sold";
  return translate(currentNumberLocale() === "da-DK" ? "da" : "en", english);
}
...
function cumulativeSeries(series: OverviewData["profitSeries"] | AnalyticsData["series"]) {
  let running = 0;
  return series.map((point) => ({ date: point.date, value: (running += Number(point.netProfitOre)) }));
}
...
function Sidebar({ section, onNavigate }: { section: Section; onNavigate: (section: Section) => void }) {
  ...
  return <aside className="track-sidebar">
    ...
    <div className="track-private"><span /> {t("Private workspace")}</div>
    <nav className="track-navigation" aria-label="Tracker navigation">{navigation.map((item, index) =>
      <button className={section === item.id ? "active" : ""} type="button" key={item.id} onClick={() => onNavigate(item.id)}><span>0{index + 1}</span>{t(item.label)}</button>)}</nav>
    <div className="track-sidebar-spacer" />
    <button className={section === "calculator" ? "track-calculator-link active" : "track-calculator-link"} type="button" onClick={() => onNavigate("calculator")}><span>07</span>{t("Calculator")}</button>
    <TrackerLanguageSelector /><p className="track-sidebar-foot">{t("Reverlo internal")}<br />{t("DKK workspace")}</p>
  </aside>;
}
...
function LoadingScreen() {
  const { t } = useTrackerI18n();
  return <div className="track-loading" aria-label={t("Loading private tracker")}><div className="track-loading-metrics">{[1, 2, 3, 4].map((item) => <i key={item} />)}</div><div className="track-loading-panels"><i /><i /></div></div>;
}
...
function Dialog({ title, kicker, children, onClose, wide = false }: { title: string; kicker: string; children: ReactNode; onClose: () => void; wide?: boolean }) {
  ...
  return <div className="track-dialog-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className={`track-dialog ${wide ? "track-dialog-wide" : ""}`} role="dialog" aria-modal="true" aria-labelledby="track-dialog-title">
      <header><div><p className="track-kicker">{t(kicker)}</p><h2 id="track-dialog-title">{t(title)}</h2></div><button type="button" className="track-dialog-close" onClick={onClose} aria-label={t("Close dialog")}>×</button></header>{children}
    </section>
  </div>;
}
...
export function PurchaseForm({ onClose, onSaved }: { onClose: () => void; onSaved: () => Promise<void> }) {
...
export function SaleForm({ products, onClose, onSaved }: { products: TrackerProduct[]; onClose: () => void; onSaved: () => Promise<void> }) {
...
function Overview({ data, onNavigate, onSale }: { data: OverviewData; onNavigate: (section: Section) => void; onSale: () => void }) {
  ...
  return <><SectionHeader kicker="Command centre" title="Overview" detail="Trading performance, actual operating spend and capital still in stock." actions={<button className="track-button-primary" type="button" onClick={onSale}>＋ {t("Record sale")}</button>} />
    <div className="track-metric-grid"><Metric index="01" label={t("Net profit")} value={formatCompactMoney(data.metrics.netProfitOre)} detail={t("Trading profit − operating expenses")} tone={data.metrics.netProfitOre < 0 ? "negative" : "positive"} /><Metric index="02" label={t("Trading profit")} value={formatCompactMoney(data.metrics.tradingProfitOre)} detail={t("Sales after trading costs")} tone={data.metrics.tradingProfitOre < 0 ? "negative" : "positive"} /><Metric index="03" label={t("Operating expenses")} value={formatCompactMoney(data.metrics.operatingExpensesOre)} detail={t("Expenses + recorded subscriptions")} /><Metric index="04" label={t("Revenue")} value={formatCompactMoney(data.metrics.revenueOre)} detail={t("Gross product sales")} /></div>
    <div className="track-capital-strip"><div><span>{t("Inventory value")}</span><strong>{formatMoney(data.metrics.inventoryValueOre)}</strong><small>{t("Remaining landed cost")}</small></div><div><span>{t("Cash invested")}</span><strong>{formatMoney(data.metrics.cashInvestedOre)}</strong><small>{t("Lifetime purchase outlay")}</small></div><button type="button" onClick={() => onNavigate("expenses")}><span>{t("Operating ledger")}</span><strong>{t("Open expenses →")}</strong><small>{t("Review actual business spend")}</small></button></div>
    <div className="track-overview-grid"><section className="track-panel track-chart-panel"><div className="track-panel-heading"><div><p className="track-kicker">{t("Bottom line")}</p><h2>{t("Net profit over time")}</h2></div><span>ALL</span></div>{series.length ? <><ProfitChart points={series} label={t("Cumulative net profit after operating expenses")} /><div className="track-chart-axis"><span>{formatDate(series[0].date)}</span><strong>{formatMoney(series.at(-1)?.value ?? 0)}</strong><span>{formatDate(series.at(-1)?.date ?? series[0].date)}</span></div></> : <EmptyState code="00" title={t("No realised result yet")} detail={t("Sales and actual operating expenses build the chart.")} action={<button type="button" className="track-text-button" onClick={onSale}>{t("Record a sale →")}</button>} />}</section>
      <section className="track-panel"><div className="track-panel-heading"><div><p className="track-kicker">{t("Ledger")}</p><h2>{t("Recent activity")}</h2></div><button type="button" className="track-text-button" onClick={() => onNavigate("transactions")}>{t("View trading →")}</button></div>{data.recentActivity.length ? <div className="track-activity-list">{data.recentActivity.map((entry) => { const outgoing = entry.kind !== "SALE"; return <article key={entry.id}><span className={`track-activity-mark ${entry.kind.toLowerCase().replace("_", "-")}`}>{entry.kind === "SALE" ? "↗" : entry.kind === "PURCHASE" ? "↓" : "−"}</span><div><strong>{entry.title}</strong><small>{entry.detail}</small></div><div><strong className={outgoing ? "negative" : ""}>{outgoing ? "−" : ""}{formatMoney(entry.amountOre)}</strong><small>{formatDate(entry.occurredAt)}</small></div></article>; })}</div> : <EmptyState code="00" title={t("No activity yet")} detail={t("Trading and operating entries will form your private ledger.")} />}</section></div>
    <section className="track-panel track-snapshot"><div className="track-panel-heading"><div><p className="track-kicker">{t("Stock position")}</p><h2>{t("Inventory snapshot")}</h2></div><button type="button" className="track-text-button" onClick={() => onNavigate("inventory")}>{t("Open inventory →")}</button></div>{data.inventorySnapshot.length ? <div className="track-snapshot-grid">{data.inventorySnapshot.map((product) => { const target = productTarget(product); const potential = potentialProfit(product); return <article key={product.id}><div><span className={statusClass(product.status)}>{statusLabel(product.status)}</span><strong>{product.name}</strong><small>{product.remainingQuantity} / {product.quantity} {t("remaining")}</small></div><div><small>{t("Target")}</small><strong>{target === null ? "—" : formatMoney(target)}</strong></div><div><small>{t("Potential")}</small><strong className={potential !== null && potential < 0 ? "negative" : "positive"}>{potential === null ? "—" : formatMoney(potential)}</strong></div></article>; })}</div> : <EmptyState code="00" title={t("Inventory is empty")} detail={t("Add your first product or record a purchase to begin.")} action={<button className="track-text-button" type="button" onClick={() => onNavigate("inventory")}>{t("Add inventory →")}</button>} />}</section>
  </>;
}
...
function Inventory({ products, onAdd, onEdit, onDelete }: { products: TrackerProduct[]; onAdd: () => void; onEdit: (product: TrackerProduct) => void; onDelete: (product: TrackerProduct) => void }) {
  ...
  return <><SectionHeader kicker="Stock ledger" title="Inventory" detail="Every batch, unit and expected return in one controlled view." actions={<button className="track-button-primary" type="button" onClick={onAdd}>＋ {t("Add item")}</button>} />
    <div className="track-toolbar"><label className="track-search"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("Search product or supplier")} aria-label={t("Search product or supplier")} /></label><div className="track-filter-group">{(["ALL", ...trackerStatuses] as const).map((status) => <button type="button" className={filter === status ? "active" : ""} onClick={() => setFilter(status)} key={status}>{status === "ALL" ? t("All") : statusLabel(status)}<span>{status === "ALL" ? products.length : products.filter((product) => product.status === status).length}</span></button>)}</div></div>
    <section className="track-table-panel">{visible.length ? <div className="track-table-scroll"><table className="track-data-table"><thead><tr><th>{t("Product")}</th><th>{t("Stock")}</th><th>{t("Purchase")}</th><th>{t("Target")}</th><th>{t("Status")}</th><th>{t("Potential")}</th><th><span className="sr-only">Actions</span></th></tr></thead><tbody>{visible.map((product) => { const target = productTarget(product); const potential = potentialProfit(product); const cost = remainingCost(product); return <tr key={product.id}><td><strong>{product.name}</strong><small>{product.supplier || t("No supplier")} · {formatDate(product.purchaseDate)}</small></td><td><strong>{product.remainingQuantity} / {product.quantity}</strong><small>{t("remaining")}</small></td><td><strong>{formatMoney(product.purchasePriceOre)}</strong><small>{formatMoney(cost)} open cost</small></td><td><strong>{target === null ? "—" : formatMoney(target)}</strong><small>{product.listingPriceOre !== null ? "listing" : product.expectedSalePriceOre !== null ? "expected" : "not set"}</small></td><td><span className={statusClass(product.status)}>{statusLabel(product.status)}</span></td><td><strong className={potential !== null && potential < 0 ? "negative" : "positive"}>{potential === null ? "—" : formatMoney(potential)}</strong><small>{potential === null ? "Set a target" : `${formatPercent(potential, cost)} ROI`}</small></td><td><div className="track-row-actions"><button type="button" onClick={() => onEdit(product)}>{t("Edit")}</button><button type="button" className="danger" onClick={() => onDelete(product)}>{t("Delete")}</button></div></td></tr>; })}</tbody></table></div> : <EmptyState code="00" title={products.length ? "No matching inventory" : "Your inventory is ready"} detail={products.length ? "Adjust the search or status filter." : "Add the first product batch to start tracking stock and potential profit."} action={!products.length ? <button className="track-button-primary" type="button" onClick={onAdd}>＋ {t("Add item")}</button> : undefined} />}</section>
  </>;
}

export function Transactions({ transactions, onPurchase, onSale }: { transactions: TrackerTransaction[]; onPurchase: () => void; onSale: () => void }) {
...
function Analytics({ data, period, onPeriod }: { data: AnalyticsData; period: AnalyticsPeriod; onPeriod: (period: AnalyticsPeriod) => void }) {
  const { t } = useTrackerI18n();
  const cumulative = cumulativeSeries(data.series); const maxComparison = Math.max(1, data.totals.revenueOre, data.totals.tradingCostsOre, data.totals.operatingExpensesOre);
  return <><SectionHeader kicker="Performance layer" title="Analytics" detail="Trading performance and real operating spend, without forecasted subscription costs." actions={<div className="track-periods">{(["30D", "90D", "YTD", "ALL"] as const).map((item) => <button className={period === item ? "active" : ""} type="button" key={item} onClick={() => onPeriod(item)}>{item}</button>)}</div>} />
    <div className="track-analytics-metrics"><Metric index="01" label={t("Revenue")} value={formatCompactMoney(data.totals.revenueOre)} detail={`${data.totals.unitsSold} units sold · ${period}`} /><Metric index="02" label={t("Trading profit")} value={formatCompactMoney(data.totals.tradingProfitOre)} detail={`${formatPercent(data.totals.tradingProfitOre, data.totals.revenueOre)} trading margin`} tone={data.totals.tradingProfitOre < 0 ? "negative" : "positive"} /><Metric index="03" label={t("Operating expenses")} value={formatCompactMoney(data.totals.operatingExpensesOre)} detail={t("Actual recorded spend")} /><Metric index="04" label={t("Net profit")} value={formatCompactMoney(data.totals.netProfitOre)} detail={t("Trading profit − operating expenses")} tone={data.totals.netProfitOre < 0 ? "negative" : "positive"} /></div>
    <div className="track-analytics-grid"><section className="track-panel track-chart-panel"><div className="track-panel-heading"><div><p className="track-kicker">{t("Bottom line")}</p><h2>{t("Net profit over time")}</h2></div><span>{period}</span></div>{cumulative.length ? <><ProfitChart points={cumulative} label={`${t("Net profit over time")} · ${period}`} /><div className="track-chart-axis"><span>{formatDate(cumulative[0].date)}</span><strong>{formatMoney(cumulative.at(-1)?.value ?? 0)}</strong><span>{formatDate(cumulative.at(-1)?.date ?? cumulative[0].date)}</span></div></> : <EmptyState code="00" title={t("No financial activity in this period")} detail={t("Choose another period or record a sale or expense.")} />}</section><section className="track-panel track-comparison"><div className="track-panel-heading"><div><p className="track-kicker">{t("Economic bridge")}</p><h2>{t("Revenue and costs")}</h2></div></div><div className="track-comparison-bars"><article><div><span>{t("Revenue")}</span><strong>{formatMoney(data.totals.revenueOre)}</strong></div><i><b style={{ width: `${(data.totals.revenueOre / maxComparison) * 100}%` }} /></i></article><article className="cost"><div><span>{t("Trading costs")}</span><strong>{formatMoney(data.totals.tradingCostsOre)}</strong></div><i><b style={{ width: `${(data.totals.tradingCostsOre / maxComparison) * 100}%` }} /></i></article><article className="operating"><div><span>{t("Operating expenses")}</span><strong>{formatMoney(data.totals.operatingExpensesOre)}</strong></div><i><b style={{ width: `${(data.totals.operatingExpensesOre / maxComparison) * 100}%` }} /></i></article></div><dl><div><dt>{t("Trading profit")}</dt><dd>{formatMoney(data.totals.tradingProfitOre)}</dd></div><div><dt>{t("Operating expenses")}</dt><dd>−{formatMoney(data.totals.operatingExpensesOre)}</dd></div><div><dt>{t("Net profit")}</dt><dd className={data.totals.netProfitOre >= 0 ? "positive" : "negative"}>{formatMoney(data.totals.netProfitOre)}</dd></div></dl></section></div>
    <section className="track-table-panel track-performance"><div className="track-panel-heading"><div><p className="track-kicker">{t("Product performance")}</p><h2>{t("What is actually working")}</h2></div><span>{data.products.length}</span></div>{data.products.length ? <div className="track-table-scroll"><table className="track-data-table"><thead><tr><th>{t("Product")}</th><th>{t("Units sold")}</th><th>{t("Revenue")}</th><th>{t("Profit")}</th><th>{t("Margin")}</th><th>{t("ROI")}</th></tr></thead><tbody>{data.products.map((product, index) => <tr key={product.productName}><td><strong>{product.productName}</strong><small>Rank {String(index + 1).padStart(2, "0")}</small></td><td><strong>{product.unitsSold}</strong></td><td><strong>{formatMoney(product.revenueOre)}</strong></td><td><strong className={product.profitOre >= 0 ? "positive" : "negative"}>{formatMoney(product.profitOre)}</strong></td><td><strong>{formatPercent(product.profitOre, product.revenueOre)}</strong></td><td><strong>{formatPercent(product.profitOre, product.costBasisOre)}</strong></td></tr>)}</tbody></table></div> : <EmptyState code="00" title={t("No product performance yet")} detail={t("Sales will build a simple product ranking here.")} />}</section>
  </>;
}
...
function TrackerRoot() {
  ...
  const loadCore = useCallback(async (initial = false) => {
    ...
    try { const [overviewResult, inventoryResult, transactionResult, expenseResult, vatResult] = await Promise.all([
      ...
      fetch("/api/track/expenses", { cache: "no-store", credentials: "same-origin" }).then((response) => responseJson<ExpensesData>(response)),
      fetch("/api/track/vat", { cache: "no-store", credentials: "same-origin" }).then((response) => responseJson<VatData>(response)),
  ...
  return <main className="track-root"><Sidebar section={section} onNavigate={navigate} /><section className="track-main">{error ? <div className="track-global-error" role="alert"><span>!</span><p>{t(error)}</p><button type="button" onClick={() => void refresh()}>{t("Retry")}</button></div> : null}{refreshing ? <span className="track-refreshing">{t("Refreshing ledger…")}</span> : null}{loading ? <><SectionHeader kicker="Private workspace" title="Loading tracker" detail="Preparing the DKK ledger and inventory position." /><LoadingScreen /></> : section === "overview" ? <Overview data={overview} onNavigate={navigate} onSale={() => setDialog({ kind: "transaction", type: "SALE" })} /> : section === "inventory" ? <Inventory products={inventory} onAdd={() => setDialog({ kind: "transaction", type: "PURCHASE" })} onEdit={(product) => setDialog({ kind: "inventory", product })} onDelete={(product) => setDialog({ kind: "delete", product })} /> : section === "transactions" ? <TrackerTransactions transactions={transactions} onCompose={(type, transaction) => setDialog({ kind: "transaction", type, transaction })} onRefresh={refresh} /> : section === "expenses" ? <TrackerExpenses data={expenses} onRefresh={refresh} /> : section === "vat" ? <TrackerVat data={vat} onRefresh={refresh} /> : section === "analytics" ? <Analytics data={analytics} period={period} onPeriod={setPeriod} /> : <Calculator />}</section>
    ...
    {dialog?.kind === "transaction" ? <TrackerTransactionDialog type={dialog.type} transaction={dialog.transaction} products={inventory} onClose={() => setDialog(null)} onSaved={refresh} /> : null}
...
export default function TrackerApp() {
  return <TrackerI18nProvider><TrackerRoot /></TrackerI18nProvider>;
}
