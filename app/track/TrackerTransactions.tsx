...
import { calculateVatAmounts } from "../../lib/tracker-accounting";
...
function oreInput(value: number | null | undefined) { return value === null || value === undefined ? "" : (value / 100).toFixed(2); }
...
export function TrackerTransactionDialog({ type, transaction, products, onClose, onSaved }: {
  ...
  const [price, setPrice] = useState(oreInput(initialPrice));
  const [shipping, setShipping] = useState(oreInput(transaction?.enteredShippingOre ?? transaction?.shippingOre ?? 0));
  const [fee, setFee] = useState(oreInput(transaction?.feeOre ?? 0));
  const [promoted, setPromoted] = useState(oreInput(transaction?.promotedFeeOre ?? 0));
  const [other, setOther] = useState(oreInput(transaction?.otherCostsOre ?? 0));
  ...
  const preview = useMemo(() => {
    ...
    try {
      ...
      const estimatedCostBasis = selected ? selected.purchasePriceOre * parsedQuantity + Math.floor(selected.purchaseShippingOre * parsedQuantity / selected.quantity) : 0;
      const totalCostsOre = estimatedCostBasis + feeOre + promotedFeeOre + enteredShippingOre + otherCostsOre;
      return { ...amounts, costBasisOre: estimatedCostBasis, totalCostsOre, profitOre: amounts.revenueOre - totalCostsOre };
    } catch { return null; }
  }, [fee, other, price, priceMode, promoted, quantity, rate, selected, shipping, transaction, treatment, type]);
    ...
    try {
      ...
      await responseJson(await fetch("/api/track/transactions", {
        ...
        body: JSON.stringify({
          ...
          feeOre, promotedFeeOre, otherCostsOre, occurredAt: values.get("occurredAt"), notes: values.get("notes"),
