...
async function responseJson<T>(response: Response) {
  const value = await response.json() as T & { error?: string; errorCode?: string };
  if (!response.ok) throw new Error(value.errorCode ?? value.error ?? "TRACKER_REQUEST_FAILED");
  return value;
}
...
export function TrackerVat({ data, onRefresh }: { data: VatData; onRefresh: () => Promise<void> }) {
  ...
  async function remove(id: string) {
    ...
    } catch (removeError) { setError(t(removeError instanceof Error ? removeError.message : "TRACKER_REQUEST_FAILED")); setDialog(null); }
  }
