...
export const trackerTransactions = sqliteTable(
  "tracker_transactions",
  {
    ...
    notes: text("notes").notNull().default(""),
    enteredUnitPriceOre: integer("entered_unit_price_ore"),
    enteredShippingOre: integer("entered_shipping_ore"),
    enteredTotalPriceOre: integer("entered_total_price_ore"),
...
export const trackerVatSettlements = sqliteTable(
  "tracker_vat_settlements",
  {
    id: text("id").primaryKey(),
  ...
  (table) => [
    index("idx_tracker_vat_settlements_date").on(table.occurredAt),
    check("tracker_vat_settlements_amount_positive", sql`${table.amountOre} > 0`),
  ],
);
...
export const trackerEmailImports = sqliteTable(
  "tracker_email_imports",
  {
    ...
    emailDate: text("email_date"), originalSubject: text("original_subject").notNull().default(""), attachmentFingerprint: text("attachment_fingerprint"), receivedAt: text("received_at").notNull().default(sql`CURRENT_TIMESTAMP`), textBody: text("text_body").notNull().default(""),
  ...
  (table) => [
    ...
    uniqueIndex("idx_tracker_email_imports_attachment_fingerprint").on(table.attachmentFingerprint).where(sql`${table.attachmentFingerprint} IS NOT NULL`),
