...
export type SourceDocumentAmount = Pick<EmailMoney, "minor" | "currency" | "source" | "provenance"> & { kind: "LINE_TOTAL" | "UNIT_PRICE" };
...
export type EmailPurchaseReview = { supplier: string; purchaseDate: string; fxRate: string; orderNumber?: string; receiptNumber?: string; invoiceNumber?: string; currency?: string; documentTotals?: Record<string, string>; items: Array<{
  sourceItemId?: string | null; sourceDocumentAmount?: SourceDocumentAmount | null; name: string; quantity: number; unitPriceOre: number | null; shippingOre: number | null; supplierCountry: string;
...
type Candidate = { source: string; kind: "INVOICE" | "RECEIPT" | "EMAIL" | "OTHER"; priority: number; supplier: string | null; supplierSource: string | null; orderNumber: string | null; receiptNumber: string | null; invoiceNumber: string | null; purchaseDate: string | null; subtotal: EmailMoney | null; subtotalSemantic: "NET" | "GENERIC" | null; shipping: EmailMoney | null; discount: EmailMoney | null; total: EmailMoney | null; amountPaid: EmailMoney | null; vatAmount: EmailMoney | null; vatRateBps: number | null; items: ParsedEmailItem[] };
...
function reference(text: string, labels: string[]) {
  const label = labels.map(escaped).join("|"); const expression = new RegExp(`^\\s*(?:${label})(?:\\s+(?:number|nummer|nr\\.?|no\\.?))?\\s*[:#-]?\\s*([A-Z0-9][A-Z0-9\\-/]{2,})\\s*$`, "i");
  const matches = [...new Set(text.split(/\r?\n/).map((line) => clean(line, 500).match(expression)?.[1]).filter((value): value is string => Boolean(value)))]; return matches.length === 1 ? matches[0] : null;
}
...
function candidateFromText(text: string, source: string, kind: Candidate["kind"], subject = "", supplier: string | null = null, supplierSource: string | null = null): Candidate {
  ...
  const vatLine = text.split(/\r?\n/).find((value) => /^\s*(?:vat|moms)\b/i.test(value)); const rate = vatLine?.match(/\(?\s*(\d{1,2}(?:[.,]\d{1,2})?)\s*%\s*(?:incl\.)?\)?/i); const parsedSupplier = supplier ?? supplierFromText(text); const parsedItems = itemsFromText(text, source); const netSubtotal = moneyAfter(text, ["total excluding tax", "subtotal excluding tax", "net total"], source); const genericSubtotal = moneyAfter(text, ["subtotal", "delsum"], source);
  return { source, kind, priority: priority(kind), supplier: parsedSupplier, supplierSource: parsedSupplier ? (supplierSource ?? (source.includes("receipt") ? "stripe_receipt_label" : `${source}_supplier`)) : null, receiptNumber, invoiceNumber, orderNumber, purchaseDate: documentedDate(text), subtotal: netSubtotal ?? genericSubtotal, subtotalSemantic: netSubtotal ? "NET" : genericSubtotal ? "GENERIC" : null, shipping: moneyAfter(text, ["shipping", "delivery", "fragt"], source), discount: moneyAfter(text, ["discount", "rabat"], source), total: moneyAfter(text, ["amount paid", "grand total", "total paid", "total", "i alt"], source), amountPaid: moneyAfter(text, ["amount paid"], source), vatAmount: moneyAfter(text, ["vat", "moms"], source), vatRateBps: rate ? Math.round(Number(rate[1].replace(",", ".")) * 100) : null, items: parsedItems.length ? parsedItems : stripeReceiptItems(text, source, parsedSupplier) };
}
...
function grossSubtotal(candidate: Candidate) { return candidate.subtotalSemantic === "GENERIC" && candidate.subtotal && candidate.total && sameValue(candidate.subtotal) === sameValue(candidate.total); }
function reconcileSubtotal(candidates: Candidate[]) {
  const values = candidates.filter((candidate) => candidate.subtotal).map((candidate) => ({ candidate, value: candidate.subtotal! })); const net = values.filter(({ candidate }) => candidate.subtotalSemantic === "NET" || !grossSubtotal(candidate));
  if (!net.length) return choose<EmailMoney>(candidates, "subtotal", "CONFLICTING_SUBTOTAL");
  const unique = [...new Set(net.map(({ value }) => sameValue(value)))]; if (unique.length > 1) return { value: null as EmailMoney | null, conflict: "CONFLICTING_SUBTOTAL", source: null };
  const winner = net.sort((left, right) => right.candidate.priority - left.candidate.priority)[0]; return { value: winner.value, conflict: null, source: winner.candidate.source };
}
...
export function initialEmailPurchaseReview(parsed: ParsedPurchaseEmail): EmailPurchaseReview {
  ...
  return { supplier: parsed.supplier ?? "", purchaseDate: parsed.purchaseDate ?? "", fxRate: "", orderNumber: parsed.orderNumber ?? "", receiptNumber: parsed.receiptNumber ?? "", invoiceNumber: parsed.invoiceNumber ?? "", currency: parsed.currency ?? "", documentTotals: { subtotal: parsed.subtotal ? `${parsed.subtotal.minor / 100}` : "", shipping: parsed.shipping ? `${parsed.shipping.minor / 100}` : "", discount: parsed.discount ? `${parsed.discount.minor / 100}` : "", vat: parsed.vatAmount ? `${parsed.vatAmount.minor / 100}` : "", total: parsed.total ? `${parsed.total.minor / 100}` : "", amountPaid: parsed.amountPaid ? `${parsed.amountPaid.minor / 100}` : "" }, items: parsed.items.map((item) => { const source = item.lineTotal ?? item.unitAmount; return { name: item.name ?? "", quantity: item.quantity ?? 0, sourceDocumentAmount: source ? { minor: source.minor, currency: source.currency, source: source.source, provenance: source.provenance, kind: item.lineTotal ? "LINE_TOTAL" : "UNIT_PRICE" } : null, unitPriceOre: parsed.currency === "DKK" ? item.unitAmount?.minor ?? null : null, shippingOre: oneItem && parsed.currency === "DKK" ? parsed.shipping?.minor ?? 0 : null, supplierCountry: "", priceMode: "", vatTreatment: "", vatRateBps: null, inputVatOre: null, outputVatOre: null, deductibleVatOre: null }; }) };
}
