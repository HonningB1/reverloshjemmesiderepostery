...
function forwardedMessage(text: string) {
  ...
  const messages = markers.flatMap((marker, index) => { const headers = lines.slice(marker + 1, Math.min(markers[index + 1] ?? lines.length, marker + 25)); const from = headers.find((line) => /^\s*(?:from|fra)\s*:/i.test(line))?.replace(/^\s*(?:from|fra)\s*:\s*/i, "").trim(); const subject = headers.find((line) => /^\s*(?:subject|emne)\s*:/i.test(line))?.replace(/^\s*(?:subject|emne)\s*:\s*/i, "").trim(); return from || subject ? [{ from: from?.slice(0, 320) ?? "", subject: subject?.slice(0, 500) ?? "" }] : []; });
...
export default {
  async email(message, env): Promise<void> {
    ...
    const attachments = [] as Array<{ name: string; contentType: string; size: number | null; sha256?: string; text?: string; extractionStatus?: string; issue?: string | null; pages?: number | null; extractedChars?: number | null }>;
    for (const attachment of (parsed.attachments ?? []).slice(0, MAX_ATTACHMENTS)) {
      ...
      const extraction = await extractPdfText(content); attachments.push({ name, contentType, size, ...extraction, extractedChars: extraction.text.length });
    }
